...
As we can see in the following graph, we achieved a nice convergence for
...
In the following graphs we check for autocorrelation between each of the
coefficients estimated, as we can see we had a good convergence.
...
In the following graphs we can see posterior probabilities distributions
for estimated parameters, as we can see none of them contains $0$, that
means that all of them have different effects on survival.
...
```{r, fig.height=4}
mcmc_areas(mod_spline, regex_pars = "Inter*|spline*", prob = 0.90, prob_outer = 0.95)
```
