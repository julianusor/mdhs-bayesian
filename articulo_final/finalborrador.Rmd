...
The main objective of the study was to predict the mortality of
people in African countries. Using data from the DHS and functions from the *mortDHS* package, functions were created for any given country within the set of DHS surveys, a useful model is generated to make predictions of survival and evaluate the effects associated with it, a measurement that cannot be made from non-parametric models such as Kaplan-Meier (as initially proposed in the study).
Specifically, a discrimination is obtained between the differences in the survival of each person depending on their sex and country, these predictions are made from a Bayesian approach using the *Rstanarm* package, which uses the Stan software to make its estimates.
...
```{r, out.width=100}
#knitr::include_graphics("articulo_final/Graficas articulo final/grafica de prueba.png", error = F)
```
